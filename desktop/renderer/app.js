...
async function init() {
  ...
  const jobs = await api.getAppliedJobs();
  renderHistory(jobs);
...
function populateForm(config) {
  ...
  if (config.openai?.model) {
    const option = $('openai-model').querySelector(`option[value="${config.openai.model}"]`);
...
async function startAutomation() {
  if (isRunning) return;
  const ok = await api.startAutomation();
  if (ok) {
    isRunning = true;
...
async function stopAutomation() {
  if (!isRunning) return;
  const ok = await api.stopAutomation();
  if (ok) {
...
async function continueLogin() {
  const ok = await api.continueLogin();
  if (ok) {
...
function renderHistory(jobs) {
  ...
  count.textContent = `Total: ${jobs.length} job${jobs.length !== 1 ? 's' : ''}`;
...
function setupIPCListeners() {
  ...
  api.onLoginStatus((status) => {
    if (status === 'started') {
      ...
      $('login-btn').disabled = false;
      $('send-enter-btn').disabled = !(isRunning || false);
    }
  });
